...
dnl@id $Id: RSSH_CHECK_OMNIORB.m4,v 1.15 2003-03-31 18:33:55 rssh Exp $
dnl
...
AC_CHECK_HEADER( omniORB3/CORBA.h, omni_3=yes , omni_3=no, )
if test "x$omni_3" = "xno"
then
 AC_CHECK_HEADER(omniORB4/CORBA.h, omni_4=yes, omni_4=no, )
fi

if test "x$omni_3" = "xyes"
then
  OMNI_VERSION=3
  omni=yes
else
 if test "x$omni_4" = "xyes" 
 then
  OMNI_VERSION=4
  omni=yes
 else
...
then
  ...
  if test ! -r "$ORB_LIBDIR/libomniORB$OMNI_VERSION.so"
  then
  ...
  OMNI_LIBDIRS="-L$ORB_LIBDIR"
  OMNI_LIBS="-lomnithread"
  svLIBS=$LIBS
  LIBS="$OMNI_LIBDIRS $OMNI_LIBS $LIBS"
...
,omni_mutex my_mutex,
  ...
  OMNI_LIBS="-lomniORB$OMNI_VERSION -lomniDynamic$OMNI_VERSION $OMNI_LIBS"
  AC_CHECK_LIB(tcpwrapGK,host_access, OMNI_LIBS="$OMNI_LIBS  -ltcpwrapGK")
  LIBS="$OMNI_LIBDIRS $OMNI_LIBS $svLIBS"
  AC_CACHE_CHECK("whether we can link with omniORB$OMNI_VERSION",
    rssh_cv_check_omniORBlib,
    AC_TRY_LINK(
#include <omniORB$OMNI_VERSION/CORBA.h>
,CORBA::ORB_var orb,
  ...
  then
    ...
    ORB_LIBDIRS=$OMNI_LIBDIRS
    ORB_LIBS=$OMNI_LIBS
    AC_SUBST(OMNI_LIBDIRS)
    AC_SUBST(OMNI_LIBS)
    AC_SUBST(ORB_LIBDIRS)
    AC_SUBST(ORB_LIBS)
  fi
  LIBS=$svLIBS
...
then
  ...
  if test "$OMNI_VERSION" = "4"
  then
     IDLFLAGS="$IDLFLAGS -bcxx -I$OMNI_ROOT/share/idl"
  else
  ...
  CORBA_H="omniORB$OMNI_VERSION/CORBA.h"
  AC_DEFINE_UNQUOTED(CORBA_H,<$CORBA_H>)

  COSNAMING_H="omniORB$OMNI_VERSION/Naming.hh"
