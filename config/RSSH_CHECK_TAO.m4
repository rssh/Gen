...
then
 ...
 then
  ...
  then
   svLIBS=$LIBS
   svLDFALGS=$LDFLAGS
   TAO_LIBDIRS="-L$ACE_ROOT/ace"
   LDFLAGS="$LDFLAGS -L$ACE_ROOT/ace"
   YAD_CHECK_INCLUDE_LIB([#include <tao/corba.h>],TAO,CORBA::ORB_var orb, TAO_LIBS="-lTAO -lACE",tao_libs=no,-lACE)
   ...
   then
     ...
     LIBS="$TAO_LIBDIRS $TAO_LIBS $svLIBS"
     YAD_CHECK_INCLUDE_LIB([#include <tao/corba.h>
#include <tao/PortableServer/PortableServer.h>],TAO_PortableServer,PortableServer::ObjectId_var oid = PortableServer::string_to_ObjectId("myObj"), TAO_LIBS="-lTAO_PortableServer $TAO_LIBS")
   fi
   LIBS="$TAO_LIBDIRS $TAO_LIBS $svLIBS"
   YAD_CHECK_INCLUDE_LIB([#include <orbsvcs/Time_Utilities.h>],TAO_Svc_Utils,ORBSVCS_Time::zero(), TAO_LIBS="-lTAO_Svc_Utils $TAO_LIBS")
   ...
   then
    ...
    YAD_CHECK_INCLUDE_LIB([#include <tao/IORTable/IORTable.h>],TAO_IORTable,TAO_IORTable_Initializer::init(), TAO_LIBS="-lTAO_IORTable $TAO_LIBS")
   fi
  fi
...
#include <$COSNAMING_H>],TAO_CosNaming,CORBA::Object_var obj; CosNaming::NamingContext_ptr nc = CosNaming::NamingContext::_narrow(obj),have_taoCosNaming=yes,have_taoCosNaming=no)
  ...
  ORB_LIBDIRS=$TAO_LIBDIRS
  ORB_LIBS=$TAO_LIBS
  AC_SUBST(TAO_LIBDIRS)
  AC_SUBST(TAO_LIBS)
